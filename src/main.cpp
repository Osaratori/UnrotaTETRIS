...
int main(void)
{
    ...
    //Board base
    Entity b_state[230];
    bool needNewBlock = true, runOnce = true, needReset = false, shouldLeaveGame = false, isLose = false, setting_level_need_reset = false;
    ...
    int random = rand()%19 + 1, bg_move = 0, bg_moveCounter = 0,
        ...
        lines = 0, Gridcell = 48, changeStateCounter = 0, justPause, score,                 //game utils
        current_index_1 = 0, current_index_2 = 0, current_index_3 = 0, current_index_4 = 0, //index savers
        lineDestroyed = 0, levelDevideLine = 0, level = 0, setting_level = 0,
        egg_1 = 0, egg_2 = 5;
    ...
    while (!WindowShouldClose() && !shouldLeaveGame)
    {
        ...
        switch (currentTitle)
        {
            ...
            case game:
                ...
                DrawTexture(Box, 32, 300, WHITE); //Lines box
                DrawTexture(Box, 32, 450, WHITE); //Score box
                DrawTexture(ThinBox, 32, 600, WHITE); //Level box
                DrawTexture(NextBox, 768, 300, WHITE);
                DrawText("LINES", 32 + (200 - MeasureText("LINES", 20)) / 2, 320, 20, WHITE);
                DrawText((scoreText.substr(egg_1, egg_2)).c_str(), 32 + (200 - MeasureText((scoreText.substr(egg_1, egg_2)).c_str(), 20)) / 2, 470, 20, WHITE);
                DrawText("NEXT", 768 + (200 - MeasureText("NEXT", 20)) / 2, 315, 20, WHITE);
                DrawText(std::to_string(lines).c_str(), 32 + (200 - MeasureText(std::to_string(lines).c_str(), 50)) / 2, 360, 50, WHITE);
                DrawText(std::to_string(score).c_str(), 32 + (200 - MeasureText(std::to_string(score).c_str(), 50)) / 2, 510, 50, WHITE);
                DrawText(("Level - " + std::to_string(level)).c_str(), 50, 617, 20, WHITE);
                DrawTextureRec(BlocksMap, Rectangle{0, (float)180 * (random - 1), 180, 180}, Vector2{778, 355}, WHITE);
                DrawFPS(10, 10);                                                                                //FPS
